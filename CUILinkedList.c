...
void InsertAtPos(PPNODE Head,int no,int iPos)
{
	...
	PNODE temp=*Head;
	size=Count(*Head);
	if((iPos<1)||(iPos>size+1))  //For invalid Position
	{
		printf("Invalid Position\n");
		return;
	}
	if(iPos==1)  //Insert if User enter 1st Position
	{
		InsertFirst(Head,no);
	}
	else if(iPos==size+1)  //If user enter greter than list input
	{
		InsertLast(Head,no);
	}
...
void DeleteAtPos(PPNODE Head,int iPos)
{
	...
	PNODE temp=*Head;
	PNODE targeted=NULL;
	if((iPos<1)||(iPos<size))  //For invalid Position
	{
		printf("Invalid Position\n");
		return;
	}
	...
	else if(iPos==size)  //If user enter greter than list input
	{
		DeleteLast(Head);
	}
	else     //In Between position
